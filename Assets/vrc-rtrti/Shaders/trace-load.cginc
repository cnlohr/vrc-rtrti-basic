// An optimized version of trace.cginc, which uses .Load instead of tex2D.
...
float4 CoreTrace( float3 eye, float3 dir )
{
	...
	while( i++ < 255 && ptr.y > 0 )
	{
		...
		if( hit && maxcorner.a > 0)
		{
			...
			//float3 N =  _GeoTex.Load( int3( ptr + float2( 1, 1 ), 0  ) );
			float4 v0 = _GeoTex.Load( int3( ptr + float2( 1, 1 ), 0 ) );
			float4 v1 = _GeoTex.Load( int3( ptr + float2( 2, 0 ), 0 ) );
			float4 v2 = _GeoTex.Load( int3( ptr + float2( 2, 1 ), 0 ) );
...
{
	// Do triangle intersection. If not, set to no intersection.
	float4 v0 = _GeoTex.Load( int3( ptr + float2( 0, 0 ), 0 ) );
	float4 v1 = _GeoTex.Load( int3( ptr + float2( 1, 0 ), 0 ) );
	float4 v2 = _GeoTex.Load( int3( ptr + float2( 2, 0 ), 0 ) );
	float4 n0 = _GeoTex.Load( int3( ptr + float2( 0, 1 ), 0 ) );
	float4 n1 = _GeoTex.Load( int3( ptr + float2( 1, 1 ), 0 ) );
	float4 n2 = _GeoTex.Load( int3( ptr + float2( 2, 1 ), 0 ) );
